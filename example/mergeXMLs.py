...
xml_files = [x for x in os.listdir(folder_path) if x.endswith(".xml")]
print("merging xml files ... ", xml_files)
sizeDownLimit = 3
sizeUpLimit = 30
...
for p in tqdm(xml_files):
    t = etree.parse(os.path.join(folder_path, p))
    r = t.getroot()
...
tree.write(os.path.join(folder_path, 'STR_data.xml'), pretty_print=True, xml_declaration=True, encoding='utf-8')
